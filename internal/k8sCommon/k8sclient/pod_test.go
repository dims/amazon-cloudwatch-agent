...
func TestPodClient_Init(t *testing.T) {
	client := fake.NewSimpleClientset()
	podClient := &podClient{}

	podClient.Init(client)
	defer podClient.Shutdown()

	if !podClient.inited {
		t.Errorf("Expected podClient to be initialized")
	}
}

func TestPodClient_Shutdown(t *testing.T) {
	client := fake.NewSimpleClientset()
	podClient := &podClient{}
	podClient.Init(client)
	...
	if podClient.inited {
		t.Errorf("Expected podClient to be shutdown")
	}
}

func TestPodClient_handlePodAdd(t *testing.T) {
	client := fake.NewSimpleClientset()
	podClient := &podClient{}
	podClient.Init(client)
	defer podClient.Shutdown()
	...
	podClient.handlePodAdd(pod)

	if podClient.namespaceToRunningPodNumMap["default"] != 1 {
		t.Errorf("Expected 1 running pod in 'default' namespace, got %d", podClient.namespaceToRunningPodNumMap["default"])
	}
}

func TestPodClient_handlePodUpdate(t *testing.T) {
	client := fake.NewSimpleClientset()
	podClient := &podClient{}
	podClient.Init(client)
	defer podClient.Shutdown()
	...
	podClient.handlePodUpdate(oldPod, newPod)
...
func TestPodClient_handlePodDelete(t *testing.T) {
	client := fake.NewSimpleClientset()
	podClient := &podClient{}
	podClient.Init(client)
	defer podClient.Shutdown()
	...
	podClient.handlePodAdd(pod)
	podClient.handlePodDelete(pod)

	if podClient.namespaceToRunningPodNumMap["default"] != 0 {
		t.Errorf("Expected 0 running pods in 'default' namespace, got %d", podClient.namespaceToRunningPodNumMap["default"])
	}
}
...
func TestPodClient_NamespaceToRunningPodNum(t *testing.T) {
	...
	client := fake.NewSimpleClientset(podArray...)
	podClient := &podClient{}
	podClient.Init(client)
	defer podClient.Shutdown()
	...
	resultMap := podClient.NamespaceToRunningPodNum()
